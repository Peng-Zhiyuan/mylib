...
//			Debug.LogError("overwriteBuildSetting " + settingName + " " + newValue + " " + buildConfigName);
			foreach( KeyValuePair<string, XCBuildConfiguration> buildConfig in buildConfigurations ) {
...
//				}
		...
		// zhiyuan.peng
		// new method
		// if Group not exist, return null
		public PBXVariantGroup GetExistVariantGroup( string name, string path = null, PBXGroup parent = null )
		{
			if( string.IsNullOrEmpty( name ) )
				return null;
