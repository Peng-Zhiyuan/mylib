...
//	}
	...
	/// </summary>
	[HideInInspector][SerializeField] Vector4 mEdgeCutoff = Vector4.zero;
	public Vector4 edgeCutoff
	{
		get
		{
			return mEdgeCutoff;
	...
	void SimpleFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		uvs.Add(new Vector2(u.x + mEdgeCutoff.w / width, u.y + mEdgeCutoff.z / height ));
		uvs.Add(new Vector2(u.x + mEdgeCutoff.w / width, u.w - mEdgeCutoff.x / height));
		uvs.Add(new Vector2(u.z - mEdgeCutoff.y / width, u.w - mEdgeCutoff.x / height));
		uvs.Add(new Vector2(u.z - mEdgeCutoff.y / width, u.y + mEdgeCutoff.z / height));
	...
	void SlicedFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		for (int x = 0; x < 3; ++x)
		{
			...
			for (int y = 0; y < 3; ++y)
			{
				...
				uvs.Add(new Vector2(mTempUVs[x].x + mEdgeCutoff.w / width, mTempUVs[y].y + mEdgeCutoff.z / height ));
				uvs.Add(new Vector2(mTempUVs[x].x + mEdgeCutoff.w / width, mTempUVs[y2].y - mEdgeCutoff.x / height));
				uvs.Add(new Vector2(mTempUVs[x2].x - mEdgeCutoff.y / width, mTempUVs[y2].y - mEdgeCutoff.x / height));
				uvs.Add(new Vector2(mTempUVs[x2].x - mEdgeCutoff.y / width, mTempUVs[y].y + mEdgeCutoff.z / height));
