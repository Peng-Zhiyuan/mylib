...
public class UIButton : UIButtonColor
{
	...
	/// </summary>
	private delegate void MonoDelegate(MonoBehaviour mono);
	private MonoDelegate action;
	...
	protected virtual void OnClick ()
	{
		if (current == null && isEnabled)
		{
			...
			if(current.onClick != null && current.onClick.Count >0)
			{
				EventDelegate.Execute(onClick);
                /*
				if (!HotfixManager.Instance.TryDoHotfix(HotfixPoint.ButtonMessage, HotfixManager.GetButtonHotfixArugument(current.name,current.onClick[0]),action,current.onClick[0].target))
					EventDelegate.Execute(onClick);
          	   */               
			}
			current = null;
...
//
//	private void Execute(GameObject go = null)
//	{
//		EventDelegate.Execute(onClick);
//	}
	/// <summary>
